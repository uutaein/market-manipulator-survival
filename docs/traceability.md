...
| FEAT-003 | Immediate Run failure | PRD v0.1.5; ADR-0008 | SRS v0.1.0, v0.1.1, v0.1.4, v0.1.6 | [run_lifecycle.feature](../feature/run/run_lifecycle.feature) | TC-FAIL-001 | SPEC Accepted / Gherkin Drafted / Step Definitions Drafted / First Playable Wired / TC Planned | Budget exhaustion, surveillance 100, and critical price collapse now mark the Run failed and route Intraday to Final Settlement F-grade. |
...
| IMP-023 | Immediate failure scene routing | SPEC v0.1.0 sections 5, 9 | `src/game/GameSession.ts`, `src/game/scenes/IntradayScene.ts` | First Playable Wired | Adds budget/surveillance/collapse failure checks, Run failed state updates, Final Settlement record save, and Intraday-to-Final routing. |
| IMP-024 | Intraday candle, volume, and money flow | SPEC v0.1.0 sections 4, 14 | `src/game/GameSession.ts`, `src/game/scenes/IntradayScene.ts`, `src/game/dom/intradayOverlays.ts` | First Playable Wired | Adds Day-local fictional price/volume history, 6-second candle aggregation, target band, crash line, current price marker, volume bars, fake OHLCV volume impulse, manual-action-specific candle patterns, and a quote/position/money-flow readout with current price, average entry, held units, spent/recovered budget, total account value, total P&L, and unrealized P/L. Normalized position value uses asset influence resistance so acquisition cost and P&L display stay consistent. Candle and volume rendering now uses a `lightweight-charts` DOM overlay. |
