...
| FEAT-002 | Day phase flow | PRD v0.1.5; ADR-0007; ADR-0012 | SRS v0.1.0, v0.1.2 | [run_lifecycle.feature](../feature/run/run_lifecycle.feature) | TC-FLOW-001 | SPEC Accepted / Gherkin Drafted / Step Definitions Drafted / Scene Wiring Partial / TC Planned | Morning News, Market Briefing, Pre-open selection, Opening Approval, Intraday entry, Day Settlement, and next-Day routing are wired in Phaser scenes. |
...
| FEAT-011 | Intraday 360-second operation | PRD v0.1.5; ADR-0007 | SRS v0.1.0, v0.1.1, v0.1.2 | [tick_simulation.feature](../feature/intraday/tick_simulation.feature) | TC-INTRA-001 | SPEC Accepted / Gherkin Drafted / Step Definitions Drafted / Domain Implemented / Scene Wiring Partial / TC Planned | 360-second timer and pause/resume behavior exist in `src/domain/intraday/`; Intraday scene now displays the initial timer value. |
...
| FEAT-017 | Retail Swarm | PRD v0.1.5; ADR-0006 | SRS v0.1.1, v0.1.3 | [retail_swarm.feature](../feature/intraday/retail_swarm.feature) | TC-SWARM-001 | SPEC Accepted / Gherkin Drafted / Step Definitions Drafted / Domain Implemented / Scene Wiring Partial / TC Planned | Swarm state, token model, warning/panic flags, and risk effects exist in `src/domain/intraday/retailSwarm.ts`; Intraday scene now renders abstract token swarms and applies transition risk effects. |
| FEAT-018 | Market Board | PRD v0.1.5; ADR-0016 | SRS v0.1.0, v0.1.3, v0.1.6 | [market_board.feature](../feature/market/market_board.feature) | TC-MARKET-001 | SPEC Accepted / Gherkin Drafted / Step Definitions Drafted / Domain Implemented / Scene Wiring Partial / TC Planned | 1 detailed player entry, 2 same-sector peers, 5 representative simplified assets, and news-priority selection exist in `src/domain/market/marketBoard.ts`; Intraday scene now refreshes player price and simplified non-player movement. |
...
| IMP-009 | Market Board domain module | SPEC v0.1.0 sections 6, 7 | `src/domain/market/marketBoard.ts` | Domain Implemented | Adds 8-slot board selection, selected asset detail entry, same-sector peers, news-affected representative priority, and simplified non-player movement summaries. |
...
| IMP-015 | Intraday tick/action scene wiring | SPEC v0.1.0 sections 6, 7 | `src/game/GameSession.ts`, `src/game/scenes/IntradayScene.ts` | Scene Wiring Partial | Adds one-second Intraday ticking, displayed price/timer updates, manual action buttons, action result feedback, and cooldown display. |
...
| IMP-018 | Auto Card scene wiring | SPEC v0.1.0 sections 7, 8 | `src/game/GameSession.ts`, `src/game/scenes/IntradayScene.ts`, `src/domain/balancing/autoCardValues.ts` | Scene Wiring Partial | Adds 90/180/270-second reward timing, paused card choice state, reward application, periodic card effects, owned card display, recent effect display, and choice buttons. |
...
| IMP-022 | Market Board live scene wiring | SPEC v0.1.0 sections 6, 7 | `src/domain/market/marketBoard.ts`, `src/game/GameSession.ts`, `src/game/scenes/IntradayScene.ts` | Scene Wiring Partial | Adds simplified non-player board advancement, live player price display, and per-second Market Board text refresh while preserving the 1 detailed + 7 simplified MVP scope. |
