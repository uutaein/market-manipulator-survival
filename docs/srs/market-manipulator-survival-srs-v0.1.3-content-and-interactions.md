...
| Card ID | 표시명 | 역할 |
| --- | --- | --- |
| `early_positioning` | 선취매 | Day 1 또는 보유 포지션이 없을 때는 현재 예산의 10~85%, Day 2 이후 보유 포지션이 있으면 0~85% 투입 비율을 조절해 보유 비중을 늘리거나 유지하는 카드. 50% 초과는 과집중 위험 구간이다. |
...
| SRS-CONTENT-PREOPEN-007 | `선취매`는 고정 비용이 아니라 드래그형 투입 비율 UI를 사용해야 한다. Day 1 또는 보유 포지션이 없을 때는 10~85%, Day 2 이후 보유 포지션이 있으면 0~85%를 선택할 수 있어야 한다. 50% 초과 선택은 과집중 위험 구간으로 표시해야 한다. |
