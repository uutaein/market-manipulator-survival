@ui @ux @visual @playwright @tc-ux-wall-expired-001
Feature: Order-book wall expired feedback experience
  Expired fictional order-book walls must leave clear feedback and clear active-row state.

  Background:
    Given the browser game starts from a clean local state
    And the player reaches Intraday operation in a Free Mode Run
    And a fictional order-book wall expired with refundable reserve
    And the game uses the Playwright visual renderer

  Scenario: Review expired wall feedback
    When the Intraday order-book panel is rendered
    Then no active wall row indicator is shown
    And the expired wall row shows cooldown with recent expiry refund state
    And the recent wall feedback shows the expired wall and released barrier
    And the feedback shows zero remaining depth and refunded reserve
    And available wall actions remain embedded in order-book rows
