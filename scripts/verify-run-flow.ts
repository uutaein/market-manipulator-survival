...
runScenario(
  "full sell enables desk reposition while action buttons stay blocked",
  () => {
    ...
    assert.equal(
      canUseManualAction(session.intradayState, "price_push"),
      false,
    );
    assert.equal(
      canUseManualAction(session.intradayState, "liquidity_supply"),
      false,
    );
    assert.equal(
      canUseManualAction(session.intradayState, "overheat_cooldown"),
      false,
    );
    assert.equal(
      canUseManualAction(session.intradayState, "position_settlement"),
      false,
    );
    ...
    const wallResult = session.useOrderBookWall(
      "buy",
      -1,
      session.intradayState.priceChangePercent - 1,
    );
    assert.equal(wallResult.applied, false);
    assert.equal(wallResult.reason, "no_position");
    assert.equal(session.canRepositionIntradayAsset(), true);

    const repositionedState = session.repositionIntradayAsset(nextAsset.id);
    assert.equal(session.ensureRun().selectedAssetId, nextAsset.id);
    assert.equal(
      repositionedState.holdingRatio,
      intradayRepositionStartingHolding,
    );
    assert.equal(repositionedState.budget, 64 - intradayRepositionEntryCost);
    assert.equal(repositionedState.activeManualActionEffects.length, 0);

    return `manual/wall actions blocked at holding 0, then repositioned to ${nextAsset.id}`;
  },
);

runScenario("order-book walls change visible depth and responsiveness", () => {
  const buySession = startFreeIntraday();
  buySession.intradayState = tuneOrderBookTestState(
    buySession.intradayState ?? buySession.startIntraday(),
  );
  ...
  const buyLevelBefore = buyProfileBefore.levels.find(
    (level) => level.offsetPercent === -1,
  );
  const buyLevelAfter = buyProfileAfter.levels.find(
    (level) => level.offsetPercent === -1,
  );
  ...
  assert.equal(
    getLastOrderBookWallEvent(buyResult.state)?.depthDelta,
    buyResult.depthBoost,
  );
  ...
  assert.ok(
    buyProfileAfter.downwardResponsiveness <
      buyProfileBefore.downwardResponsiveness,
  );
  assert.ok(
    buyProfileAfter.executionDepth.bids.some(
      (level) =>
        level.quantity === buyLevelAfter.bidDepth && level.orderCount > 1,
    ),
  );
  ...
  assert.ok(
    shiftedBuyProfile.executionDepth.bids.some(
      (level) =>
        level.quantity === shiftedBuyLevel.bidDepth && level.orderCount > 1,
    ),
  );

  const decaySession = startFreeIntraday();
  decaySession.intradayState = tuneOrderBookTestState(
    decaySession.intradayState ?? decaySession.startIntraday(),
  );
  ...
  const decayLevelAfter = decayProfileAfter.levels.find(
    (level) => level.offsetPercent === -1,
  );
  ...
  const partiallyMeltedEffect =
    partiallyMeltedState.activeOrderBookWallEffects.find(
      (effect) =>
        effect.side === "buy" &&
        Math.abs(effect.priceChangePercent - decayWallPrice) < 0.01,
    );
  assert.ok(partiallyMeltedEffect);
  assert.ok(
    getOrderBookWallRemainingDepthBoost(partiallyMeltedEffect) <
      decayResult.depthBoost,
  );
  assert.ok(
    getOrderBookWallRemainingReservedBudget(partiallyMeltedEffect) <
      decayResult.reservedBudget,
  );
  ...
  assert.ok(
    (getLastOrderBookWallEvent(partiallyMeltedState)?.depthDelta ?? 0) < 0,
  );
  assert.equal(
  ...
  assert.equal(
    removePartialWallResult.budgetDelta,
    getOrderBookWallRemainingReservedBudget(partiallyMeltedEffect),
  );
  assert.ok(removePartialWallResult.budgetDelta > 0);
  assert.ok(removePartialWallResult.budgetDelta < decayResult.reservedBudget);
  assert.equal(
    getLastOrderBookWallEvent(removePartialWallResult.state)?.type,
    "removed",
  );
  assert.equal(
    getLastOrderBookWallEvent(removePartialWallResult.state)?.reserveDelta,
    removePartialWallResult.budgetDelta,
  );

  const meltSession = startFreeIntraday();
  meltSession.intradayState = tuneOrderBookTestState(
    meltSession.intradayState ?? meltSession.startIntraday(),
  );
  ...
  assert.equal(
    fullyMeltedState.activeOrderBookWallEffects.some(
      (effect) =>
        effect.side === "buy" &&
        Math.abs(effect.priceChangePercent - meltWallPrice) < 0.01,
    ),
    false,
  );
  ...
  const postMeltBarrierState = applyOrderBookWallPriceBarriers(
    fullyMeltedState,
    postMeltDownsideAttempt,
  );
  assert.equal(postMeltBarrierState.priceChangePercent, meltWallPrice - 4);

  buySession.intradayState = shiftedBuyState;
  const removeBuyResult = buySession.useOrderBookWall(
    "buy",
    shiftedBuyLevel.offsetPercent,
    buyWallPrice,
  );
  ...
  const repeatBuyResult = buySession.useOrderBookWall(
    "buy",
    shiftedBuyLevel.offsetPercent,
    buyWallPrice,
  );
  ...
  const secondLevelBuyPrice = getLevelPriceChangePercent(
    postRemoveBuyProfile,
    -2,
  );
  const secondLevelBuyResult = buySession.useOrderBookWall(
    "buy",
    -2,
    secondLevelBuyPrice,
  );
  ...
  assert.equal(
    cooledState.activeOrderBookWallEffects.some(
      (effect) => effect.side === "buy" && effect.offsetPercent === -2,
    ),
    false,
  );
  ...
  sellSession.intradayState = tuneOrderBookTestState(
    sellSession.intradayState ?? sellSession.startIntraday(),
  );
  ...
  const sellLevelBefore = sellProfileBefore.levels.find(
    (level) => level.offsetPercent === 1,
  );
  const sellLevelAfter = sellProfileAfter.levels.find(
    (level) => level.offsetPercent === 1,
  );
  ...
  assert.ok(
    sellProfileAfter.upwardResponsiveness <
      sellProfileBefore.upwardResponsiveness,
  );
  assert.ok(
    sellProfileAfter.executionDepth.asks.some(
      (level) =>
        level.quantity === sellLevelAfter.askDepth && level.orderCount > 1,
    ),
  );
  ...
  const buyBarrierState = applyOrderBookWallPriceBarriers(
    buyResult.state,
    downsideAttempt,
  );
  ...
  const sellBarrierState = applyOrderBookWallPriceBarriers(
    sellResult.state,
    upsideAttempt,
  );
  ...
  const partiallyMeltedSellState = tickOrderBookWallEffects(
    pressuredSellWallState,
    5,
  );
  const partiallyMeltedSellEffect =
    partiallyMeltedSellState.activeOrderBookWallEffects.find(
      (effect) =>
        effect.side === "sell" &&
        Math.abs(effect.priceChangePercent - sellWallPrice) < 0.01,
    );
  assert.ok(partiallyMeltedSellEffect);
  assert.ok(
    getOrderBookWallRemainingDepthBoost(partiallyMeltedSellEffect) <
      sellResult.depthBoost,
  );
  assert.ok(
    getOrderBookWallRemainingReservedBudget(partiallyMeltedSellEffect) <
      sellResult.reservedBudget,
  );
...
runScenario(
  "no carried position selects a new asset before the next Day pre-open",
  () => {
    const session = startFreeIntraday();
    const state = session.intradayState ?? session.startIntraday();
...
function tuneOrderBookTestState(
  state: ReturnType<GameSession["startIntraday"]>,
) {
...
function getLevelPriceChangePercent(
  ...
  const level = profile.levels.find(
    (candidate) => candidate.offsetPercent === offsetPercent,
  );
  assert.ok(level);
  return level.priceChangePercent;
}

function getLastOrderBookWallEvent(
  state: ReturnType<GameSession["startIntraday"]>,
) {
