...
| `execution/` | Local-only synthetic execution gateway and reference matching engine |
| `intraday/` | Intraday state, timer pause/resume, bounded stats, player asset price tick, order-book/depth model, local execution depth adapter, and seeded chart motion |
...
| `safety/safetyContract.ts` | Player-facing content safety checks and fictional calculation validation |
