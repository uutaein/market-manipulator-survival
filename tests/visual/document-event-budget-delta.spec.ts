...
import {
  expectCanvasReady,
  prepareDocumentEventPopupState,
  resetBrowserState,
} from "./support/canvas";

test("document event budget delta visual baseline", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("document-event-budget-delta.png", {
    animations: "disabled",
    fullPage: true,
  });
});
