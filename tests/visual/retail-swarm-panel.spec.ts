...
import { expectCanvasReady, prepareRetailSwarmPanicState, resetBrowserState } from "./support/canvas";

test("retail swarm panel visual baseline", async ({ page }) => {
  ...
  await prepareRetailSwarmPanicState(page);
  await page.waitForTimeout(1100);
  await expectCanvasReady(canvas);

  await expect(page).toHaveScreenshot("retail-swarm-panel.png", {
    animations: "disabled",
    fullPage: true
  });
});
