...
import {
  expectCanvasReady,
  prepareActiveManualActionProgressState,
  resetBrowserState,
} from "./support/canvas";

test("intraday active manual action progress visual baseline", async ({
  page,
}) => {
  ...
  await prepareActiveManualActionProgressState(page);
  await page.waitForTimeout(1200);
  await expectCanvasReady(canvas);

  await expect(page).toHaveScreenshot("intraday-manual-action-progress.png", {
    animations: "disabled",
    fullPage: true,
  });
});
