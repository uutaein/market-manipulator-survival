...
import {
  ...
  prepareContractFinalSettlementState,
  resetBrowserState,
} from "./support/canvas";

test("contract final settlement visual baseline", async ({ page }) => {
  ...
  await prepareContractFinalSettlementState(page);
  ...
  await expect(page).toHaveScreenshot("contract-final-settlement.png", {
    animations: "disabled",
    fullPage: true,
  });
});
