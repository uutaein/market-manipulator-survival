...
import {
  expectCanvasReady,
  prepareIntradaySurveillanceAlertState,
  resetBrowserState,
} from "./support/canvas";

test("intraday surveillance alert visual baseline", async ({ page }) => {
  ...
  await prepareIntradaySurveillanceAlertState(page);
  await page.waitForTimeout(900);
  await expectCanvasReady(canvas);

  await expect(page).toHaveScreenshot("intraday-surveillance-alert.png", {
    animations: "disabled",
    fullPage: true,
  });
});
