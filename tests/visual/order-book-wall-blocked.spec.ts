...
import { expectCanvasReady, prepareOrderBookWallBlockedState, resetBrowserState } from "./support/canvas";

test("order-book wall blocked visual baseline", async ({ page }) => {
  ...
  await prepareOrderBookWallBlockedState(page);
  ...
  await expect(page).toHaveScreenshot("order-book-wall-blocked.png", {
    animations: "disabled",
    fullPage: true
  });
});
