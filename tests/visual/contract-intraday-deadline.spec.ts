...
import {
  expectCanvasReady,
  prepareContractIntradayDeadlineState,
  resetBrowserState,
} from "./support/canvas";

test("contract intraday deadline pressure visual baseline", async ({
  page,
}) => {
  ...
  await prepareContractIntradayDeadlineState(page);
  await page.waitForTimeout(500);
  await expectCanvasReady(canvas);

  await expect(page).toHaveScreenshot("contract-intraday-deadline.png", {
    animations: "disabled",
    fullPage: true,
  });
});
