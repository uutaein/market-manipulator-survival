...
import {
  expectCanvasReady,
  prepareNextDayAssetSelectionState,
  resetBrowserState,
} from "./support/canvas";

test("next day asset selection visual baseline", async ({ page }) => {
  ...
  await prepareNextDayAssetSelectionState(page);
  ...
  await expect(page).toHaveScreenshot("next-day-asset-selection.png", {
    animations: "disabled",
    fullPage: true,
  });
});
