...
test("contract intraday action fit feedback visual baseline", async ({
  page,
}) => {
  ...
  await page.mouse.click(548, 620);
  await page.waitForTimeout(500);
  await expectCanvasReady(canvas);

  await expect(page).toHaveScreenshot("contract-intraday-action-fit.png", {
    animations: "disabled",
    fullPage: true,
  });
});
