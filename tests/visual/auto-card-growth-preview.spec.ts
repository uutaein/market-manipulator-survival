...
import {
  expectCanvasReady,
  prepareAutoCardGrowthPreviewState,
  resetBrowserState,
} from "./support/canvas";

test("auto card growth preview visual baseline", async ({ page }) => {
  ...
  await prepareAutoCardGrowthPreviewState(page);
  await page.waitForTimeout(1100);
  await expectCanvasReady(canvas);

  await expect(page).toHaveScreenshot("auto-card-growth-preview.png", {
    animations: "disabled",
    fullPage: true,
  });
});
