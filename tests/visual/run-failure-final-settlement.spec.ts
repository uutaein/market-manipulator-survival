...
import {
  expectCanvasReady,
  prepareRunFailureFinalSettlementState,
  resetBrowserState,
} from "./support/canvas";

test("run failure final settlement visual baseline", async ({ page }) => {
  ...
  await prepareRunFailureFinalSettlementState(page);
  await page.waitForTimeout(1250);
  await expectCanvasReady(canvas);

  await expect(page).toHaveScreenshot("run-failure-final-settlement.png", {
    animations: "disabled",
    fullPage: true,
  });
});
