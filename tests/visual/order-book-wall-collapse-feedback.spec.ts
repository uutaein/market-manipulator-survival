...
import {
  expectCanvasReady,
  prepareOrderBookWallCollapsedState,
  resetBrowserState,
} from "./support/canvas";

test("order-book wall collapse feedback visual baseline", async ({ page }) => {
  ...
  await prepareOrderBookWallCollapsedState(page);
  ...
  await expect(page).toHaveScreenshot("order-book-wall-collapse-feedback.png", {
    animations: "disabled",
    fullPage: true,
  });
});
