...
import {
  expectCanvasReady,
  prepareIntradayCrashBufferAlertState,
  resetBrowserState,
} from "./support/canvas";

test("intraday crash buffer alert visual baseline", async ({ page }) => {
  ...
  await prepareIntradayCrashBufferAlertState(page);
  await page.waitForTimeout(900);
  await expectCanvasReady(canvas);

  await expect(page).toHaveScreenshot("intraday-crash-buffer-alert.png", {
    animations: "disabled",
    fullPage: true,
  });
});
